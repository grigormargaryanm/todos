...
const Todos: FC = () => {
    ...
    const renderTodosList = useCallback(() => {
        return isLessThanMd ? (
            isLoading ? (
                Array.from({length: TODO_PAGE_SIZE}).map((_, index) => (
    ...
    return (
        <Wrapper>
            ...
            <AddTodoWrapper>
                ...
                <Button type="primary" onClick={() => setCreateEditModalOpen(true)}>Add Todo</Button>
            </AddTodoWrapper>
            {renderTodosList()}
            <Modal title="Delete Modal" open={deleteModalOpen} onOk={handleDelete} onCancel={handleCancelDelete}>
                <Text>Do you want to delete task?</Text>
            </Modal>
            <CreateEditTodo createEditModalOpen={createEditModalOpen} setCreateEditModalOpen={setCreateEditModalOpen}
                            todo={currentTodo} todos={todos} setCurrentTodo={setCurrentTodo}/>
