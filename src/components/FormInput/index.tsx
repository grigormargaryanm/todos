...
function FormInput({
  ...
  const render = () => {
    return password ? <Input.Password {...inputProps} /> : <Input {...inputProps} />
  }

  const placement = errorPlacement ? errorPlacement : screens.lg ? 'right' : 'top'

  const handleGetPopupContainer = (triggerNode: HTMLElement) => {
    return isPopupContainerBody ? document.body : (triggerNode.parentNode as HTMLElement)
  }
